...
function SiteOutputViewer({ pagina }: { pagina: SitePageOutput }) {
  return (
    <>
      <Card>
        <CardHeader><CardTitle className="text-base">SEO da página</CardTitle></CardHeader>
        <CardContent className="space-y-3">
          ...
          <CopyField label="Schema JSON-LD" value={JSON.stringify(pagina.schemaJsonLd, null, 2)} multiline code />
        </CardContent>
      </Card>
          ...
          <Tabs defaultValue={pagina.sections[0]?.tipo}>
            <TabsList className="flex-wrap h-auto">
              {pagina.sections.map((s) => (
                <TabsTrigger key={s.tipo} value={s.tipo} className="text-xs">{sectionLabel(s.tipo)}</TabsTrigger>
              ))}
            </TabsList>
            {pagina.sections.map((s) => (
      ...
      <Card>
        <CardHeader><CardTitle className="text-base">Instruções de uso</CardTitle></CardHeader>
        <CardContent>
...
function SectionPreview({ section }: { section: SiteSection }) {
  return (
    <>
      {section.headline && <CopyField label="Headline" value={section.headline} />}
      {section.subheadline && <CopyField label="Subheadline" value={section.subheadline} multiline />}
      {section.bullets && section.bullets.length > 0 && (
        <CopyField label="Bullets" value={section.bullets.join("\n")} multiline />
      )}
      {section.faq && section.faq.length > 0 && (
        <CopyField label="FAQ" value={section.faq.map((f) => `Q: ${f.pergunta}\nA: ${f.resposta}`).join("\n\n")} multiline />
      )}
      {section.cta && <CopyField label="CTA" value={`${section.cta.texto} → ${section.cta.href}`} />}
      ...
      <Tabs defaultValue="0">
        ...
        {campanha.adGroups.map((ag, i) => (
          <TabsContent key={i} value={String(i)} className="space-y-3 mt-4">
            <Card>
              <CardHeader><CardTitle className="text-sm">Palavras-chave</CardTitle></CardHeader>
            ...
            <CopyField label="15 Headlines (≤30 chars)" value={ag.headlines.map((h, k) => `${k + 1}. ${h} (${h.length})`).join("\n")} multiline />
            <CopyField label="4 Descriptions (≤90 chars)" value={ag.descriptions.map((d, k) => `${k + 1}. ${d} (${d.length})`).join("\n")} multiline />
            <CopyField label="10 Callouts (≤25 chars)" value={ag.callouts.map((c) => `${c} (${c.length})`).join("\n")} multiline />
            <CopyField label="Sitelinks" value={ag.sitelinks.map((s, k) => `${k + 1}. ${s.texto}\n   ${s.descricao1}\n   ${s.descricao2}`).join("\n\n")} multiline />
          </TabsContent>
        ))}
      </Tabs>

      <Card>
        <CardHeader><CardTitle className="text-base">Palavras-chave NEGATIVAS</CardTitle></CardHeader>
        <CardContent><CopyField label="Lista" value={campanha.palavrasNegativas.join("\n")} multiline /></CardContent>
      </Card>

      <Card>
        <CardHeader><CardTitle className="text-base">Instruções de uso</CardTitle></CardHeader>
        <CardContent><p className="text-sm whitespace-pre-wrap">{campanha.instrucoesUso}</p></CardContent>
      </Card>
    </>
  );
}
...
function MetaAdsViewer({ campanha }: { campanha: MetaAdsOutput }) {
  return (
    <>
      <Card>
        <CardHeader><CardTitle className="text-base">Conjunto de anúncios</CardTitle></CardHeader>
      ...
      <Card>
        <CardHeader><CardTitle className="text-base">Público sugerido</CardTitle></CardHeader>
        <CardContent className="space-y-2 text-sm">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1 mt-1">
              {campanha.publicoSugerido.interesses.map((i, k) => <Badge key={k} variant="secondary">{i}</Badge>)}
            </div>
          </div>
          <div>
            <strong>Comportamentos:</strong>
            <div className="flex flex-wrap gap-1 mt-1">
              {campanha.publicoSugerido.comportamentos.map((c, k) => <Badge key={k} variant="secondary">{c}</Badge>)}
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <Tabs defaultValue="0">
        <TabsList className="flex-wrap h-auto">
          {campanha.variacoes.map((v, i) => (
            <TabsTrigger key={i} value={String(i)}>Var {i + 1} · {v.angulo}</TabsTrigger>
          ))}
        </TabsList>
        {campanha.variacoes.map((v, i) => (
          <TabsContent key={i} value={String(i)} className="space-y-3 mt-4">
      ...
      <Card>
        <CardHeader><CardTitle className="text-base">Ideias de criativo</CardTitle></CardHeader>
        <CardContent>
          <ul className="text-sm space-y-2">
            {campanha.ideiasCriativos.map((i, k) => <li key={k}>· {i}</li>)}
          </ul>
        </CardContent>
      </Card>
...
function CopyField({ label, value, multiline, code }: { label: string; value: string; multiline?: boolean; code?: boolean }) {
    ...
    <div>
      <div className="flex items-center justify-between mb-1">
        ...
        <Button variant="ghost" size="sm" onClick={handleCopy}>
          {copied ? <><Check className="size-3 mr-1" /> Copiado</> : <><Copy className="size-3 mr-1" /> Copiar</>}
        </Button>
      </div>
      {multiline ? (
        <pre className={`text-xs bg-muted p-3 rounded whitespace-pre-wrap break-words ${code ? "font-mono" : ""}`}>{value}</pre>
      ) : (
