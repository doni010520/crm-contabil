...
export function ProfileWizard({ initial }: ProfileWizardProps) {
  ...
  function toggleNicho(n: Nicho) {
    ...
    if (exists) {
      update(
        "nichos",
        profile.nichos.filter((x) => x !== n)
      );
    } else if (profile.nichos.length < 3) {
      update("nichos", [...profile.nichos, n]);
    }
  }
  ...
  function toggleServico(s: Servico) {
    ...
    if (exists) {
      update(
        "servicos",
        profile.servicos.filter((x) => x !== s)
      );
    } else {
      update("servicos", [...profile.servicos, s]);
    }
  }
  ...
  function removeCase(i: number) {
    update(
      "cases",
      profile.cases.filter((_, idx) => idx !== i)
    );
  }
  ...
  function canAdvance(): boolean {
    if (step === 1) {
      return !!(profile.nome && profile.cidade && profile.crcUf && profile.crcNumero);
    }
    if (step === 2) {
      return profile.nichos.length > 0 && profile.servicos.length > 0;
    }
    if (step === 3) {
      return (
      ...
      <Card>
        <CardContent className="p-6 space-y-4">
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <Input
                    ...
                    placeholder="Contabilidade Andrade"
                  />
                </div>
                <div>
                ...
                <div>
                  ...
                  <Input
                    id="bairro"
                    value={profile.bairroPrincipal || ""}
                    onChange={(e) => update("bairroPrincipal", e.target.value)}
                    placeholder="Savassi"
                  />
                </div>
                <div>
                  ...
                  <Input
                    id="uf"
                    value={profile.estadoAtuacao}
                    onChange={(e) => update("estadoAtuacao", e.target.value.toUpperCase())}
                    placeholder="MG"
                    maxLength={2}
                  />
                </div>
                <div>
                  ...
                  <Input
                    id="crc_uf"
                    value={profile.crcUf}
                    onChange={(e) => update("crcUf", e.target.value.toUpperCase())}
                    placeholder="MG"
                    maxLength={2}
                  />
                </div>
                <div>
                  ...
                  <Input
                    id="crc_num"
                    value={profile.crcNumero}
                    onChange={(e) => update("crcNumero", e.target.value)}
                    placeholder="045812"
                  />
                </div>
                <div>
                  ...
                  <Input
                    id="anos"
                    type="number"
                    value={profile.anosMercado}
                    onChange={(e) => update("anosMercado", parseInt(e.target.value || "0"))}
                  />
                </div>
                <div>
                  <Label>Faixa de clientes ativos</Label>
                  <Select
                    value={profile.faixaClientes}
                    onValueChange={(v) => update("faixaClientes", v as EscritorioProfile["faixaClientes"])}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
          ...
          {step === 2 && (
            <>
              <div>
                <Label>Nichos principais * (selecione até 3)</Label>
                <p className="text-xs text-muted-foreground mb-2">
                  Este é o input que mais diferencia copy genérica de copy de especialista.
                </p>
                <div className="flex flex-wrap gap-2">
                  {NICHO_OPTIONS.map((n) => {
                    const selected = profile.nichos.includes(n.value);
                    return (
                      <Badge
                        key={n.value}
                ...
                <p className="text-xs text-muted-foreground mt-2">
                  Selecionados: {profile.nichos.length}/3
                </p>
              </div>
              ...
              <div className="pt-4">
                ...
                <div className="flex flex-wrap gap-2 mt-1">
                  {SERVICO_OPTIONS.map((s) => {
                    const selected = profile.servicos.includes(s.value);
                    return (
                      <Badge
                        key={s.value}
                        variant={selected ? "default" : "outline"}
              ...
              <div className="pt-4 grid sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <Select
                    value={profile.modeloPreco}
                    onValueChange={(v) => update("modeloPreco", v as EscritorioProfile["modeloPreco"])}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                ...
                {profile.modeloPreco !== "sob-consulta" && (
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        update("precoInicialMensal", e.target.value ? parseFloat(e.target.value) : undefined)
                      }
                      placeholder="397"
                    />
                  </div>
          ...
          {step === 3 && (
            <>
              <div>
                <Label>3 diferenciais reais do escritório *</Label>
                <p className="text-xs text-muted-foreground mb-2">
                  Concreto, verificável. Ex: "Contador dedicado nominal", "Resposta em 4h úteis",
              ...
              <div className="pt-4">
                ...
                <Textarea
                  ...
                  placeholder="Ex: Médico PJ ou recém-saído da CLT, faturando R$ 20k a 80k/mês, sem tempo nem paciência para contabilidade."
                  rows={3}
                />
              </div>
              ...
              <div className="pt-4">
                ...
                {profile.cases.map((c, i) => (
                  <div key={i} className="border rounded-md p-3 mb-2 grid sm:grid-cols-3 gap-2 relative">
                    <Input
                      value={c.segmento}
                      onChange={(e) => updateCase(i, "segmento", e.target.value)}
                      placeholder="Segmento (ex: clínica odontológica)"
                    />
                    <Input
                      value={c.porte}
                      onChange={(e) => updateCase(i, "porte", e.target.value)}
                      placeholder="Porte (ex: 12 funcionários)"
                    />
                    <Input
                      value={c.resultado}
                      onChange={(e) => updateCase(i, "resultado", e.target.value)}
                      placeholder="Resultado (ex: economia R$ 6,4k/mês)"
                    />
                    <Button
                      type="button"
                      variant="ghost"
          ...
          {step === 4 && (
            <>
              <div>
                ...
                <Select
                  value={profile.tomDeVoz}
                  onValueChange={(v) => update("tomDeVoz", v as EscritorioProfile["tomDeVoz"])}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="formal-consultivo">
                      Formal-consultivo (médicos, advogados, holdings)
                    </SelectItem>
                    <SelectItem value="proximo-direto">
                      Próximo-direto (varejo, restaurantes, MEI)
                    </SelectItem>
                    <SelectItem value="informal-tecnologico">
                      Informal-tecnológico (startups, infoprodutores, e-commerce)
                    </SelectItem>
                  </SelectContent>
                </Select>
              </div>
              ...
              <div className="pt-4">
                ...
                <Select
                  value={profile.ctaPrimario}
                  onValueChange={(v) =>
                    update("ctaPrimario", v as EscritorioProfile["ctaPrimario"])
                  }
                >
                  <SelectTrigger>
                    <SelectValue />
              ...
              <div className="pt-4 grid sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Input
                    id="gmb"
                    value={profile.linkGoogleMeuNegocio || ""}
                    onChange={(e) => update("linkGoogleMeuNegocio", e.target.value)}
