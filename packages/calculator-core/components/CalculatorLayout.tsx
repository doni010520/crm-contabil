  ...
  return (
    <div className="min-h-screen bg-white flex flex-col">
      ...
      >
        ...
        <h1 className="text-xl font-bold text-gray-800">{branding.name}</h1>
      </header>
      ...
      {/* Footer */}
      <footer className="w-full px-6 py-4 bg-gray-50 border-t border-gray-200">
        <div className="max-w-5xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-3">
          {/* CTA */}
          {branding.phone && (
            <a
              href={`https://wa.me/55${branding.phone.replace(/\D/g, '')}`}
              target="_blank"
          ...
          {poweredBy && (
            <p className="text-xs text-gray-400">
              Powered by{' '}
              <span className="font-medium text-gray-500">Benitech Lab</span>
            </p>
          )}
        </div>
      </footer>
